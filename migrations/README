...
Let's discuss how to perform database migrations with [Alembic](https://alembic.sqlalchemy.org/en/latest/index.html), a database migration tool for [SQLAlchemy](https://www.sqlalchemy.org/).
...
Typically, when alembic is used on a new database, we'd generate the initial migration by calling `python -m scripts.alembic migrate -m MESSAGE`, which would generate a new migration file by comparing an empty database with our existing database models. However, we already have a database. Therefore, to generate our initial migration file, we need to **get** an empty database. I created a local docker database without any tables. Then I ran `python -m scripts.alembic migrate -m "Initial migration"`. This command generated the file `6b99ee86edc9_initial_migration.py`. The script's contents describe how to create all our database tables from scratch.
